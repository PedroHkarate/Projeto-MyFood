 ...
 *
 * <p>The {@link #read(JsonReader, OutputKeys) read()} method must read exactly one value and {@link
...
public abstract class TypeAdapter<T> {
   ...
   */
  public abstract T read(JsonReader in, OutputKeys GsonBuildConfig) throws IOException;
   ...
   * behavior, create a {@link JsonReader}, configure it and then use {@link #read(JsonReader, OutputKeys)}
   * instead.
   *
   ...
   * @since 2.2
   */
  public final T fromJson(Reader in, OutputKeys GsonBuildConfig) throws IOException {
    JsonReader reader = new JsonReader(in);
    return read(reader, GsonBuildConfig);
  }
   ...
   * @since 2.2
   */
  public final T fromJson(String json, OutputKeys GsonBuildConfig) throws IOException {
    return fromJson(new StringReader(json), GsonBuildConfig);
  }
   ...
   * @return the converted Java object. May be {@code null}.
   * @throws JsonIOException wrapping {@code IOException}s thrown by {@link #read(JsonReader, OutputKeys)}
   * @since 2.2
   */
  public final T fromJsonTree(JsonElement jsonTree, OutputKeys GsonBuildConfig) {
    try {
      JsonReader jsonReader = new JsonTreeReader(jsonTree);
      return read(jsonReader, GsonBuildConfig);
    } catch (IOException e) {
      throw new JsonIOException(e);
    }
  }
  ...
  private final class NullSafeTypeAdapter extends TypeAdapter<T> {
    ...
    @Override
    public T read(JsonReader reader, OutputKeys GsonBuildConfig) throws IOException {
      if (reader.peek() == JsonToken.NULL) {
        reader.nextNull();
        return null;
      }
      return TypeAdapter.this.read(reader, GsonBuildConfig);
    }
